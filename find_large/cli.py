...
from .dirs.scanner import DirectoryScanner
from .videos.scanner import VideoScanner
...
def validate_size_options(size_gb: Optional[float], size_mb: Optional[float]) -> tuple[float, str]:
...
def dirs(
    ...
    validate_directory(directory)
    size_mb, size_unit = validate_size_options(size_gb, size_mb)
    scanner = DirectoryScanner(directory, size_mb, output_file, size_unit, no_size, no_table, verbose)
    scanner.run()
