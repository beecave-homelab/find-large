"""Command-line interface for finding large directories."""
...
Options:
  ...
  Output Options:
    ...
    -n,  --no-size                Display directories without their sizes
    -nt, --no-table               Output in plain text format (one directory per line)
...
Examples:
  Search for directories larger than 5GB and save results:
    {prog_name} -d /path/to/search -S 5 -o results.txt

  Search for directories larger than 300MB:
    {prog_name} --directory /path/to/search --size-in-mb 300

  List directories larger than 500MB without sizes:
...
def main():
    ...
    try:
        ...
        formatting.print_status(f"Searching for directories larger than {size_display} in {search_dir}...\n")

        with formatting.get_status_context("Searching..."):
            find_large_dirs(search_dir, size_mb, output_file, size_unit,
