"""End-to-end tests for videos CLI entry point."""
...
from find_large.videos.cli import main, scan_videos


class TestVideosCLIEntryPoints:
    """Test cases for videos CLI entry point."""
...
class TestScanVideosFunction:
    ...
    def test_scan_videos_defaults_to_gb(
        self, tmp_path: Path, capsys: pytest.CaptureFixture
    ) -> None:
        """Test scan_videos defaults to GB when no size specified."""
        scan_videos(str(tmp_path), None, None, None, False, False, False)
        captured = capsys.readouterr()
        assert "Searching for videos larger than" in captured.out

    def test_scan_videos_raises_on_both_size_options(self, tmp_path: Path) -> None:
        """Test scan_videos raises error when both size options provided."""
    ...
    def test_scan_videos_with_output_file(
        self, tmp_path: Path, capsys: pytest.CaptureFixture
    ) -> None:
        """Test scan_videos with output file."""
        output_file = tmp_path / "output.txt"
